...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  {
      /* TODO handle the region management (lift up sbrk, add page if needed )*/
      /*Attempt to increate limit to get space */
      struct vm_area_struct* cur_vma = get_vma_by_num(caller->mm, vmaid);
      int inc_sz = PAGING_PAGE_ALIGNSZ(size);
      int incnumpage = inc_sz / PAGING_PAGESZ;

      /* Store old_sbrk for later use */
      old_sbrk = cur_vma->sbrk;

      /* Increase limit to get space */
      ...
      */
      ...
      /* Update cur_vma's sbrk */
      cur_vma->sbrk += incnumpage * PAGING_PAGESZ;
  }
  else
  {
      /* Region found successfully */
...
int __free(struct pcb_t *caller, int vmaid, int rgid) {
    ...
    // Get the vm_area_struct corresponding to vmaid
    ...
    // Get the vm_rg_struct corresponding to rgid
    struct vm_rg_struct* rg = get_symrg_byid(mm, rgid);
    if (rg == NULL) {
        // Symbol region not found
        return -1;
    }

    // Enlist the obsoleted memory region to freerg_list
    enlist_vm_rg_node(&(vma->vm_freerg_list), rg);
    int size = rg->rg_end - rg->rg_start;
    int numpages = PAGING_PAGE_ALIGNSZ(size) / PAGING_PAGESZ;
    int fpn = PAGING_FPN(rg->rg_start);
    int pgn = PAGING_PGN(rg->rg_start);
    for (int i = 0; i < numpages; i++) {
        uint32_t pte = caller->mm->pgd[pgn + i];
        //fpn = PAGING_FPN(pte);  
        if (PAGING_PAGE_PRESENT(pte))
            MEMPHY_put_freefp(caller->mram, fpn + i);
    }
...
int pg_getpage(struct mm_struct* mm, int pgn, int* fpn, struct pcb_t* caller) {
    ...
    if (!PAGING_PAGE_PRESENT(pte)) {
        ...
        tgtfpn = PAGING_SWP(pte); // The target frame storing our variable
        ...
        /* Copy target frame from swap to memory */
        __swap_cp_page(caller->active_mswp, tgtfpn, caller->mram, vicpgn);

        /* Update page table */
        vicpte = mm->pgd[vicpgn];
        pte_set_swap(&vicpte, 0, mm->pgd[swpfpn]);
        mm->pgd[vicpgn] = vicpte;
        /* Update the online status of the target page */
